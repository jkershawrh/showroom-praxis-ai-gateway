...
`configure_praxis` role consumes the endpoint and per-order virtual key produced
by the RHDP LiteMaaS workload and creates the namespace, Praxis configuration,
model credential Secret, and OAuth cookie Secret required by the GitOps
deployment.
